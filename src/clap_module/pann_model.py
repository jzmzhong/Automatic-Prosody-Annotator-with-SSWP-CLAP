...
class Cnn14(nn.Module):
    ...
    def forward(self, input, mixup_lambda=None, device=None):
        ...
        Args:
            input (dict): (batch_size, data_length)
            mixup_lambda (_type_, optional): Mixup on spectrogram. Defaults to None.
        ...
        if not self.enable_fusion:
            ...
            x = self.spectrogram_extractor(input['waveform'].to(device=device, non_blocking=True))
        ...
        if (self.enable_fusion) and (self.fusion_type in ['daf_2d','aff_2d','iaff_2d']):
            ...
            if len(longer_list_idx) > 0:
                ...
                if local_x.size(-2) < TH:
                    local_x = torch.cat([local_x, torch.zeros((TB,TC,TH-local_x.size(-2),TW),
                                                              device=global_x.device)], dim=-2)
                else:
                    local_x = local_x[:,:,:TH,:]
...
def create_pann_model(audio_cfg, enable_fusion=False, fusion_type='None'):
    try:
        ...
        raise RuntimeError(f'Import Model for {audio_cfg.model_name} not found, '
                           f'r the audio cfg parameters are not enough.')
